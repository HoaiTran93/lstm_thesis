...
def prepare_seq2seq_data(dataset, look_back, look_ahead):
    ...
    # batch_num = int(dataX.shape[0] / 671)
    # print('batch_num:',batch_num)
    # dataX = dataX[:671 * batch_num,:,:]
    # dataY = dataY[:671 * batch_num,:,:]
...
def load_data(data_folder,look_back,look_ahead):
    ...
    print('train:',train[:, 0].reshape(-1,1).shape)
    print('validation1:',validation1[:, 0].reshape(-1,1).shape)
    print('validation2:',validation2[:, 0].reshape(-1,1).shape)
    print('test:',test[:, 0].reshape(-1,1).shape)
